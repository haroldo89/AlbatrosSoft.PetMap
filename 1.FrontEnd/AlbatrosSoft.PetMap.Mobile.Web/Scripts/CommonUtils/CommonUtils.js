...
//Muestra mensaje
CommonUtils.showMessage = function (lblMessage, messageText) {
    var lblContainer = String.format('#{0}', lblMessage);
...
//Concatena una fecha con hora y minuto
CommonUtils.joinDateHour = function (date, time) {
    if (date && time) {
        var currentHour = time.getHours();
        var currentMinute = time.getMinutes();        
        var fullDate = new Date(date.getFullYear(), date.getMonth(), date.getDate(), currentHour, currentMinute);
        return fullDate;
    }
    return new Date();
}

//Valida cuantos dias hay entre dos fechas
CommonUtils.diffDays = function (firstDate, secondDate) {
    var diffDays = 0;
